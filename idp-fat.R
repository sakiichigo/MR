...
if(FALSE){
  ...
  #writeLines(idpLists, "E:/idpList.txt")
  
  idpLists=readLines("E:/idp641-800.txt")
  for(filename in idpLists){
    idp=data.table::fread(paste0(idpPath,filename)  ,header = TRUE)
    write.csv(idp,paste0(idpCsvPath,strsplit(filename,"\\.",fixed = FALSE)[[1]][1],".h.csv"))
    rm(idp)
    gc()
  }
...
if(FALSE){
  ...
  idpists=list.files(path=idpPath, pattern=NULL, all.files=FALSE, full.names=FALSE)
  for (filename in idpists){
  
    idp=data.table::fread(paste0(idpPath,filename)  ,header = TRUE)
    thisName=strsplit(filename,"\\.",fixed = FALSE)[[1]][1]
    
    idp_outTab<-subset(idp, p_value<5e-08)
    write.csv(idp_outTab,paste0("E:/idpSubset/",idp))
    
  }
  
  idp_outTab<-subset(idp, p_value<5e-08)
  write.csv(idp_outTab,"E:/idpSubset/GCST90002916.h.csv")
  
  #colnames(idp)= as.character(unlist(idp[1,]))
  #x=10^(-(-log10(idp$`pval(-log10)`)))
  #idp$`pval(-log10)`=x
...
#用idp作暴露
workPath="D:/document/bioInfo/sacropenia-liver-0317"
dir.create(workPath)
idpPath="E:/idp/"
idpCsvPath="E:/idpCsv/"
#导出or,he,ple记录
...
for(filename in idpLists){
  thisName=strsplit(filename,"\\.",fixed = FALSE)[[1]][1]
  n=thisName
  csvFileName=paste0(idpCsvPath,thisName,".h.csv")
  expCsvName=paste0("E:/idpExposure/",thisName,".exp.csv")
  naCsvName=paste0(idpCsvPath,thisName,".h.csv.na")
  if(file.exists(naCsvName)){
    next
  }
  ####temp
  if(!file.exists(expCsvName)){
    expCsvName=paste0("E:/idpExposure/",thisName,"_buildGRCh37.exp.csv")
    if(!file.exists(expCsvName))
      next
  }
  if(!file.exists(expCsvName)){
