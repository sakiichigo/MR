...
#用exp作暴露
workPath="D:/document/bioInfo/ALPS-atherosclerosisGwas"
dir.create(workPath)

expPath="E:/anaFiles/alpsGwas/exp-8/"
outPath="E:/anaFiles/atherosclerosisGwas/tsv/"

if(TRUE){
  expPath="E:/anaFiles/atherosclerosisGwas/exp-8/"
  outPath="E:/anaFiles/alpsGwas/ALPS/"
}
#导出or,he,ple记录
saveRecord=TRUE
...
#rerun
expLists=list.files(path=expPath, pattern=NULL, all.files=FALSE, full.names=FALSE)
...
for(expfilename in expLists){
  thisName=strsplit(expfilename,"\\.",fixed = FALSE)[[1]][1]
  n=thisName
  expCsvName=paste0(expPath,thisName,".exp.csv")
  ...
  for(outfilename in outLists){
    thisName=strsplit(outfilename,"\\.",fixed = FALSE)[[1]][1]
    m=thisName
    outCsvName=paste0(outPath,thisName,".txt")
    result=tryCatch({
